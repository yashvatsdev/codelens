...
export function PRReviewsContent({ repositories }: PRReviewsContentProps) {
  ...
  const [viewState, setViewState] = useState<"history" | "form" | "result">(
    "history",
  );
  const [history, setHistory] = useState<PRReviewHistoryItem[]>([]);
  ...
  const handleViewReview = async (reviewId: number) => {
    ...
    setCopiedKeys({});
    try {
      const data = await api.getPRReview(reviewId);
      setReviewResult(data as unknown as AIPRReviewResponse);
      setActiveReviewMeta({
        repoName: data.repository_full_name || "",
        prNumber: data.pull_request_number,
        repoId: data.repository_id,
      });
    } catch (err) {
      setReviewError(
        getFriendlyErrorMessage(err, "Failed to load PR review details."),
      );
    } finally {
      setIsLoading(false);
    }
                ...
                {reviewResult.key_findings.length === 0 ? (
                  <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-8 text-center">
                    ...
                    <p className="text-xs text-zinc-400 mt-1 max-w-md mx-auto">
                      ...
                      critical bugs, vulnerabilities, or code smells in the
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto text-left">
                <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
                  ...
                  <p className="text-[11px] text-zinc-500 mt-1 leading-normal">
                    Analyzes only the files added or modified in the pull
                    request.
                  </p>
                </div>
                <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
                  ...
                  <p className="text-[11px] text-zinc-500 mt-1 leading-normal">
                    AST & pattern analyzers for Python, JavaScript, and
                    TypeScript.
                  </p>
                </div>
                <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
